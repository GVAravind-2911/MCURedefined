...
const EditHistoryModal = memo(function EditHistoryModal({
	...
	return (
		<div
			className="fixed inset-0 bg-black/85 flex items-center justify-center z-1100 p-4 animate-[fadeIn_0.2s_ease]"
		...
		>
			<div className="bg-[linear-gradient(135deg,rgba(20,20,20,0.98)_0%,rgba(30,30,30,0.98)_100%)] border border-white/20 rounded-xl w-full max-w-[600px] max-h-[80vh] overflow-hidden backdrop-blur-[20px] shadow-[0_20px_60px_rgba(0,0,0,0.6)] animate-[modalSlideIn_0.3s_ease]">
				<div className="flex items-center justify-between p-5 border-b border-white/10">
					<h2 className="font-[BentonSansBold] text-xl text-white m-0">
						Edit History
					</h2>
					<button
						className="bg-white/10 border border-white/20 text-white/70 text-xl cursor-pointer w-8 h-8 rounded-lg transition-all duration-200 flex items-center justify-center hover:bg-[rgba(236,29,36,0.2)] hover:border-[rgba(236,29,36,0.5)] hover:text-white"
						onClick={onClose}
					>
						×
					</button>
							...
							{history.map((item, index) => (
								...
								>
									<div className="flex items-center justify-between mb-3 text-sm">
										<span className="font-[BentonSansBold] text-white/90">
											Version {item.editNumber}
										</span>
