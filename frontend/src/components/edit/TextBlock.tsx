...
const FormatButton = memo(
	({
		...
		>
			{icon}
			{shortcut && (
				<span className="hidden lg:inline text-[10px] text-white/40 ml-1">
					{shortcut}
				</span>
			)}
		</button>
...
const TextBlock: React.FC<TextBlockProps> = memo(
	({ content, onChange, onDelete }) => {
		...
		const {
			...
			canRedo,
			syncExternal,
		} = useTextUndoRedo(content, onChange, {
			maxHistory: 100,
			debounceMs: 500,
		});
		...
		return (
			<div className="relative bg-[rgba(40,40,40,0.4)] border border-white/10 rounded-xl overflow-visible transition-all duration-300 animate-[fadeInBlock_0.3s_ease-out] hover:border-white/20 hover:shadow-[0_4px_20px_rgba(0,0,0,0.4)]">
				...
				{/* Toolbar */}
				<div
					className={`absolute top-0 right-0 flex flex-wrap gap-1 p-1.5 bg-linear-to-br from-[rgba(60,60,60,0.95)] to-[rgba(40,40,40,0.95)] rounded-tr-xl rounded-bl-lg border-b border-l border-white/10 z-5 transition-all duration-300 ${isFocused ? "opacity-100" : "opacity-70"} hover:opacity-100`}
				>
					{/* Undo/Redo */}
					<FormatButton
						onClick={undo}
