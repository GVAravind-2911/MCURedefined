...
export default function StepThreeSetPassword({ data, onChange, onSubmit, onBack, error, isLoading }) {
    ...
    return (
        <form onSubmit={handleSubmit} className="auth-form" style={{ maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div className="form-group">
                ...
                {data.password && (
                    <div className={`password-strength strength-${passwordStrength.label.toLowerCase()}`}>
                        <div className="strength-label" style={{ marginTop: '8px', fontWeight: '500' }}>
                            {passwordStrength.label && (
                                <>
                                    <span style={{ 
                                        ...
                                        borderRadius: '50%',
                                        backgroundColor: 
                                            passwordStrength.label === 'Weak' ? '#ef4444' : 
                                            passwordStrength.label === 'Fair' ? '#f59e0b' : 
                                            passwordStrength.label === 'Good' ? '#22c55e' : '#10b981',
                                        marginRight: '6px'
                                    }}></span>
                        ...
                        <div className="password-requirements" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                            <div className={`requirement ${passwordStrength.requirements.length ? 'met' : 'not-met'}`}>
                                <span className="requirement-icon">
                                    {passwordStrength.requirements.length ? '✓' : '○'}
                                </span>
                                <span>At least 8 characters</span>
                            </div>
                            <div className={`requirement ${passwordStrength.requirements.uppercase ? 'met' : 'not-met'}`}>
                                <span className="requirement-icon">
                                    {passwordStrength.requirements.uppercase ? '✓' : '○'}
                                </span>
                                <span>Uppercase letter</span>
                            </div>
                            <div className={`requirement ${passwordStrength.requirements.lowercase ? 'met' : 'not-met'}`}>
                                <span className="requirement-icon">
                                    {passwordStrength.requirements.lowercase ? '✓' : '○'}
                                </span>
                                <span>Lowercase letter</span>
                            </div>
                            <div className={`requirement ${passwordStrength.requirements.number ? 'met' : 'not-met'}`}>
                                <span className="requirement-icon">
                                    {passwordStrength.requirements.number ? '✓' : '○'}
                                </span>
                                <span>Number</span>
                            </div>
                            <div className={`requirement ${passwordStrength.requirements.special ? 'met' : 'not-met'}`} style={{ gridColumn: '1 / -1' }}>
                                <span className="requirement-icon">
            ...
            <div className="form-group">
                ...
                {data.password && data.confirmPassword && data.password !== data.confirmPassword && (
                    <div className="password-mismatch" style={{color: '#ef4444', fontSize: '0.85rem', marginTop: '0.5rem'}}>
            ...
            <div className="form-buttons" style={{ marginTop: '1.5rem' }}>
                ...
                <button
                    type="submit"
                    disabled={isLoading || (data.password !== data.confirmPassword) || !data.password}
