"use client";

import { useRef, useEffect, useState } from "react";
import { USER_ROLES } from "@/types/UserManagementTypes";

interface UserToolbarProps {
	searchValue: string;
	setSearchValue: (value: string) => void;
	searchField: "email" | "name";
	setSearchField: (field: "email" | "name") => void;
	filterRole: string;
	setFilterRole: (role: string) => void;
	filterBanned: string;
	setFilterBanned: (banned: string) => void;
	onSearch: (e: React.FormEvent) => void;
	onCreateUser: () => void;
	setCurrentPage: (page: number) => void;
}

export default function UserToolbar({
	searchValue,
	setSearchValue,
	searchField,
	setSearchField,
	filterRole,
	setFilterRole,
	filterBanned,
	setFilterBanned,
	onSearch,
	onCreateUser,
	setCurrentPage,
}: UserToolbarProps) {
	const [searchFieldOpen, setSearchFieldOpen] = useState(false);
	const [roleFilterOpen, setRoleFilterOpen] = useState(false);
	const [statusFilterOpen, setStatusFilterOpen] = useState(false);
	
	const searchFieldRef = useRef<HTMLDivElement>(null);
	const roleFilterRef = useRef<HTMLDivElement>(null);
	const statusFilterRef = useRef<HTMLDivElement>(null);

	// Close dropdowns when clicking outside
	useEffect(() => {
		const handleClickOutside = (event: MouseEvent) => {
			if (searchFieldRef.current && !searchFieldRef.current.contains(event.target as Node)) {
				setSearchFieldOpen(false);
			}
			if (roleFilterRef.current && !roleFilterRef.current.contains(event.target as Node)) {
				setRoleFilterOpen(false);
			}
			if (statusFilterRef.current && !statusFilterRef.current.contains(event.target as Node)) {
				setStatusFilterOpen(false);
			}
		};

		document.addEventListener("mousedown", handleClickOutside);
		return () => document.removeEventListener("mousedown", handleClickOutside);
	}, []);

	const getSearchFieldLabel = () => {
		return searchField === "email" ? "Email" : "Name";
	};

	const getStatusLabel = () => {
		if (filterBanned === "true") return "Banned";
		if (filterBanned === "false") return "Active";
		return "All Status";
	};

	const getRoleLabel = () => {
		if (!filterRole) return "All Roles";
		return USER_ROLES[filterRole as keyof typeof USER_ROLES]?.label || filterRole;
	};

	return (
		<div className="user-management-toolbar">
			<form onSubmit={onSearch} className="um-search-section">
				{/* Custom Search Field Dropdown */}
				<div className="custom-dropdown" ref={searchFieldRef}>
					<button
						type="button"
						className="dropdown-trigger"
						onClick={() => setSearchFieldOpen(!searchFieldOpen)}
					>
						<span>{getSearchFieldLabel()}</span>
						<svg
							className={`dropdown-arrow ${searchFieldOpen ? "open" : ""}`}
							width="12"
							height="12"
							viewBox="0 0 12 12"
						>
							<path d="M2 4L6 8L10 4" stroke="currentColor" strokeWidth="2" fill="none" />
						</svg>
					</button>
					{searchFieldOpen && (
						<div className="dropdown-menu">
							<button
								type="button"
								className={`dropdown-item ${searchField === "email" ? "selected" : ""}`}
								onClick={() => {
									setSearchField("email");
									setSearchFieldOpen(false);
								}}
							>
								Email
							</button>
							<button
								type="button"
								className={`dropdown-item ${searchField === "name" ? "selected" : ""}`}
								onClick={() => {
									setSearchField("name");
									setSearchFieldOpen(false);
								}}
							>
								Name
							</button>
						</div>
					)}
				</div>

				<div className="um-search-input-wrapper">
					<svg className="um-search-icon" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
						<circle cx="11" cy="11" r="8" />
						<line x1="21" y1="21" x2="16.65" y2="16.65" />
					</svg>
					<input
						type="text"
						placeholder={`Search by ${searchField}...`}
						value={searchValue}
						onChange={(e) => setSearchValue(e.target.value)}
						className="um-search-input"
					/>
					{searchValue && (
						<button
							type="button"
							className="um-clear-search"
							onClick={() => {
								setSearchValue("");
								setCurrentPage(1);
							}}
						>
							<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
								<line x1="18" y1="6" x2="6" y2="18" />
								<line x1="6" y1="6" x2="18" y2="18" />
							</svg>
						</button>
					)}
				</div>
				<button type="submit" className="um-search-btn">
					Search
				</button>
			</form>

			<div className="um-filter-section">
				{/* Role Filter Dropdown */}
				<div className="custom-dropdown" ref={roleFilterRef}>
					<button
						type="button"
						className="dropdown-trigger"
						onClick={() => setRoleFilterOpen(!roleFilterOpen)}
					>
						<span>{getRoleLabel()}</span>
						<svg
							className={`dropdown-arrow ${roleFilterOpen ? "open" : ""}`}
							width="12"
							height="12"
							viewBox="0 0 12 12"
						>
							<path d="M2 4L6 8L10 4" stroke="currentColor" strokeWidth="2" fill="none" />
						</svg>
					</button>
					{roleFilterOpen && (
						<div className="dropdown-menu">
							<button
								type="button"
								className={`dropdown-item ${!filterRole ? "selected" : ""}`}
								onClick={() => {
									setFilterRole("");
									setCurrentPage(1);
									setRoleFilterOpen(false);
								}}
							>
								All Roles
							</button>
							{Object.entries(USER_ROLES).map(([key, value]) => (
								<button
									type="button"
									key={key}
									className={`dropdown-item ${filterRole === key ? "selected" : ""}`}
									onClick={() => {
										setFilterRole(key);
										setCurrentPage(1);
										setRoleFilterOpen(false);
									}}
								>
									<span className={`role-dot ${value.color}`} />
									{value.label}
								</button>
							))}
						</div>
					)}
				</div>

				{/* Status Filter Dropdown */}
				<div className="custom-dropdown" ref={statusFilterRef}>
					<button
						type="button"
						className="dropdown-trigger"
						onClick={() => setStatusFilterOpen(!statusFilterOpen)}
					>
						<span>{getStatusLabel()}</span>
						<svg
							className={`dropdown-arrow ${statusFilterOpen ? "open" : ""}`}
							width="12"
							height="12"
							viewBox="0 0 12 12"
						>
							<path d="M2 4L6 8L10 4" stroke="currentColor" strokeWidth="2" fill="none" />
						</svg>
					</button>
					{statusFilterOpen && (
						<div className="dropdown-menu">
							<button
								type="button"
								className={`dropdown-item ${!filterBanned ? "selected" : ""}`}
								onClick={() => {
									setFilterBanned("");
									setCurrentPage(1);
									setStatusFilterOpen(false);
								}}
							>
								All Status
							</button>
							<button
								type="button"
								className={`dropdown-item ${filterBanned === "false" ? "selected" : ""}`}
								onClick={() => {
									setFilterBanned("false");
									setCurrentPage(1);
									setStatusFilterOpen(false);
								}}
							>
								<span className="status-dot active" />
								Active
							</button>
							<button
								type="button"
								className={`dropdown-item ${filterBanned === "true" ? "selected" : ""}`}
								onClick={() => {
									setFilterBanned("true");
									setCurrentPage(1);
									setStatusFilterOpen(false);
								}}
							>
								<span className="status-dot banned" />
								Banned
							</button>
						</div>
					)}
				</div>

				<button
					type="button"
					className="um-create-user-btn"
					onClick={onCreateUser}
				>
					<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
						<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" />
						<circle cx="9" cy="7" r="4" />
						<line x1="19" y1="8" x2="19" y2="14" />
						<line x1="22" y1="11" x2="16" y2="11" />
					</svg>
					<span>Create User</span>
				</button>
			</div>
		</div>
	);
}
