...
export default function IndividualProject({ project }: IndividualProjectProps): JSX.Element {
  if (!project) {
    notFound();
  }
  ...
  return (
    <div className="projectinfo-container fade-in">
      ...
      <div className="projectinfo-content">
        <h1>{project.name}</h1>
        <br />
        <h3>Release Date : {moment(project.release_date).format('dddd, D MMMM, YYYY')}</h3>
        <br />
