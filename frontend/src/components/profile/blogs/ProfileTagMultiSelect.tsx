...
const TagMultiSelect = memo(
	({ selectedTags, availableTags, onChange }: ProfileTagMultiSelectProps) => {
		...
		return (
			<div className="relative z-10 max-md:w-full" ref={tagDropdownRef}>
				...
				{showTagFilter && (
					<div
						className="absolute top-full left-0 mt-2 bg-[#1a1a1a] border border-white/20 rounded-xl overflow-hidden min-w-[220px] max-h-[300px] overflow-y-auto scrollbar-thin z-20 shadow-lg max-md:w-full"
						role="menu"
					>
						{availableTags.length > 0 ? (
							<>
								<div className="flex items-center justify-between py-2.5 px-4 border-b border-white/10 bg-white/5">
									<span className="text-white/60 text-sm font-[BentonSansRegular]">
								...
								{availableTags.map((tag) => (
									...
									>
										<span
											className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all duration-200 ${selectedTags.includes(tag) ? "bg-[#ec1d24] border-[#ec1d24]" : "border-white/40"}`}
										>
