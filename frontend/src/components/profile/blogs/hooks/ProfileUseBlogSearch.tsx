import { useState, useEffect, useCallback, type RefObject, useMemo, useRef } from 'react';
...
const useBlogSearch = ({
    ...
    const fetchBlogs = useCallback(async (page: number, query = "", tags: string[] = [], author = "") => {
        setLoading(true);
        ...
        try {
          ...
          if (apiUrl.includes('/api/user/liked')) {
            ...
            const params = [];
            params.push(`type=${apiUrl.includes('type=reviews') ? 'reviews' : 'blogs'}`);
      ...
      const handleSearch = useCallback(
        (query: string, tags: string[], author: string, page = 1) => {
          ...
          const urlWithParams = params.toString() ? `${currentPath}?${params}` : currentPath;
      ...
      // biome-ignore lint/correctness/useExhaustiveDependencies: <explanation>
        ...
        if (page === currentPage || page < 1 || page > blogContext.totalPages) return;
      ...
      }, [currentPage, searchQuery, selectedTags, selectedAuthor, pathname, blogContext.totalPages, containerRef, cachedPages, fetchBlogs, blogContext]);

  const handleTagClick = useCallback((e: React.MouseEvent<HTMLButtonElement>, tag: string) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const getPaginationNumbers = useCallback(() => {
    ...
    for (let i = Math.max(2, currentPage - 1); i <= Math.min(blogContext.totalPages - 1, currentPage + 1); i++) {
      pages.push(i);
    }
