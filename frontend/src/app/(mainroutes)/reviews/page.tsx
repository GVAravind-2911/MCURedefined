...
export default async function Reviews(): Promise<React.ReactElement> {
	...
	if ("hasError" in blogData) {
		return <ErrorMessage title={blogData.title} reasons={blogData.reasons} />;
	}

	if ("hasError" in tags) {
		return <ErrorMessage title={tags.title} reasons={tags.reasons} />;
	}

	if ("hasError" in authors) {
		return <ErrorMessage title={authors.title} reasons={authors.reasons} />;
	}
			...
			<div className="blog-hero">
				...
				<div className="hero-content">
					...
					<p className="hero-description">
						Explore in-depth reviews of Marvel Cinematic Universe films, shows,
						and streaming content. Read critical analysis, ratings, and fan
						perspectives on your favorite Marvel productions.
					</p>
				</div>
			</div>
			<BlogProvider
