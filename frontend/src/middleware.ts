...
export async function middleware(request: NextRequest) {
  ...
  // Different parts you can access:
	...
	if (pathname.includes('blogs') || pathname.includes('release-slate') || pathname.includes('reviews')) {
...
export const config = {
	matcher: ["/manage/:path*","/blogs/:path*","/reviews/:path*","/release-slate/:path*","/auth/reset-password"],
};