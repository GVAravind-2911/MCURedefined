...
const parseHtmlToJsx = (html: string) => {
    ...
    const parseTextContent = (text: string, parentKey = '') => {
        return text.split('\n').map((line, index) => {
            ...
            return React.createElement(React.Fragment, { key }, [
                line,
                index < text.split('\n').length - 1 && React.createElement('br', { key: `${key}-br` })
            ].filter(Boolean));
        });
    };
    ...
    const convertToJsx = (htmlString: string, parentKey = '') => {
        ...
        let lastIndex = 0;
        htmlString.replace(regex, (match, tag, attrs, content, selfClosingTag, selfClosingAttrs, index) => {
            ...
            // Parse attributes
            const attrRegex = /(\w+)=["'](.*?)["']/g;
            const props: Record<string, string | number | boolean | React.CSSProperties> = { key: elementKey };
