...
Handles image uploads for blog posts (thumbnails and content images).
...
# R2 folders for blog images
BLOG_THUMBNAILS_FOLDER = "blog-thumbnails"
BLOG_CONTENT_FOLDER = "blog-images"

# Default image for blogs
DEFAULT_BLOG_IMAGE = {"link": "https://i.imgur.com/JloNMTG.png", "key": ""}


class BlogImageService:
    """
    Service for handling blog post images.
    ...
    def upload_thumbnail(base64_string: str) -> dict[str, str]:
        ...
        logger.debug(
            "Uploading blog thumbnail to R2",
            **{"folder": BLOG_THUMBNAILS_FOLDER}
        )
        
        try:
            result = storage.upload_base64_image(base64_string, folder=BLOG_THUMBNAILS_FOLDER)
            logger.info(
                "Blog thumbnail uploaded successfully",
                **{"image.key": result.get("key", "")}
            )
            return result
        except Exception as e:
            logger.error(
                f"Failed to upload blog thumbnail: {str(e)}",
    ...
    def upload_content_image(base64_string: str) -> dict[str, str]:
        ...
        logger.debug(
            "Uploading blog content image to R2",
            **{"folder": BLOG_CONTENT_FOLDER}
        )
        
        try:
            result = storage.upload_base64_image(base64_string, folder=BLOG_CONTENT_FOLDER)
            logger.info(
                "Blog content image uploaded successfully",
                **{"image.key": result.get("key", "")}
            )
            return result
        except Exception as e:
            logger.error(
                f"Failed to upload blog content image: {str(e)}",
                extra={"error.type": type(e).__name__, "error.message": str(e)}
            )
            raise
    ...
    def delete_image(key: str) -> bool:
        ...
        # Validate the key belongs to blog folders for safety
        valid_prefixes = [BLOG_THUMBNAILS_FOLDER + "/", BLOG_CONTENT_FOLDER + "/"]
        if not any(key.startswith(prefix) for prefix in valid_prefixes):
            logger.warning(
                "Attempted to delete image outside blog folders",
                **{"image.key": key}
            )
            return False
        
        logger.debug(
            "Deleting blog image from R2",
            **{"image.key": key}
        )
        ...
        try:
            ...
            if result:
                ...
                logger.warning(
                    "Blog image deletion returned False",
                    **{"image.key": key}
                )
            return result
        except Exception as e:
            logger.error(
                f"Failed to delete blog image: {str(e)}",
    ...
    def process_thumbnail(image_data: Optional[dict[str, Any]], use_default_if_not_base64: bool = False) -> dict[str, str]:
        ...
        if not image_data or not isinstance(image_data, dict):
            ...
            return DEFAULT_BLOG_IMAGE.copy()
        ...
        if isinstance(link, str) and link.startswith("data:image"):
            ...
            return BlogImageService.upload_thumbnail(link)
    ...
    def process_content_image(image_data: Optional[dict[str, Any]]) -> dict[str, str]:
        ...
        if isinstance(link, str) and link.startswith("data:image"):
            ...
            return BlogImageService.upload_content_image(link)
    ...
    def process_content_blocks(content: list[dict]) -> list[dict]:
        ...
        for i, block in enumerate(content):
            if block.get("type") == "image":
                ...
                block["content"] = BlogImageService.process_content_image(block.get("content", {}))
        ...
        for block in content:
            if block.get("type") == "image":
                ...
                if isinstance(image_content, dict):
                    ...
                    if key and key.startswith(BLOG_CONTENT_FOLDER + "/"):
                        keys.append(key)
        return keys
    ...
    def extract_thumbnail_key(thumbnail_path: Any) -> Optional[str]:
        ...
        if key and isinstance(key, str) and (key.startswith(BLOG_THUMBNAILS_FOLDER + "/") or key.startswith(BLOG_CONTENT_FOLDER + "/")):
            return key
        return None
    ...
    def cleanup_orphaned_images(
        ...
        old_content_keys = set(BlogImageService.extract_image_keys_from_content(old_content or []))
        new_content_keys = set(BlogImageService.extract_image_keys_from_content(new_content or []))
        ...
        for key in orphaned_content_keys:
            logger.debug(f"Deleting orphaned blog content image: {key}")
            if BlogImageService.delete_image(key):
                deleted += 1
            else:
                failed += 1
        
        # Check thumbnail change
        old_thumb_key = BlogImageService.extract_thumbnail_key(old_thumbnail)
        new_thumb_key = BlogImageService.extract_thumbnail_key(new_thumbnail)
        old_thumb_link = BlogImageService.extract_thumbnail_link(old_thumbnail)
        new_thumb_link = BlogImageService.extract_thumbnail_link(new_thumbnail)
        ...
        if thumbnail_changed and old_thumb_key:
            logger.info(f"Deleting orphaned blog thumbnail: {old_thumb_key}")
            if BlogImageService.delete_image(old_thumb_key):
                deleted += 1
            else:
                failed += 1
        elif old_thumb_key:
        ...
        if deleted > 0 or failed > 0:
            logger.info(
                f"Blog image cleanup: deleted {deleted}, failed {failed}",
    ...
    def cleanup_all_images(
        ...
        """
        Delete all images associated with a blog post (for deletion).
        ...
        content_keys = BlogImageService.extract_image_keys_from_content(content or [])
        for key in content_keys:
            logger.debug(f"Deleting blog content image: {key}")
            if BlogImageService.delete_image(key):
                deleted += 1
            else:
                failed += 1
        ...
        # Delete thumbnail
        thumb_key = BlogImageService.extract_thumbnail_key(thumbnail)
        if thumb_key:
            logger.debug(f"Deleting blog thumbnail: {thumb_key}")
            if BlogImageService.delete_image(thumb_key):
                deleted += 1
            else:
                failed += 1
        ...
        if deleted > 0 or failed > 0:
            logger.info(
                f"Blog deletion image cleanup: deleted {deleted}, failed {failed}",
                **{"cleanup.deleted": deleted, "cleanup.failed": failed}
            )
